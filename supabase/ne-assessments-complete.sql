-- NE ASSESSMENTS complete — run entire file in Supabase SQL Editor
...
-- PREREQ: run supabase/assessments-rpcs.sql first — or use ne-assessments-complete.sql (all-in-one).
-- Safe to re-run.

-- Backfill technician_id from legacy tech_id column if present
DO $$
BEGIN
  IF EXISTS (
    ...
    WHERE technician_id IS NULL AND tech_id IS NOT NULL;
  END IF;
END $$;

-- Drop duplicate rows so (technician_id, date) can be unique (keeps one row per pair)
DELETE FROM public.assessments a
WHERE a.technician_id IS NOT NULL
  AND a.ctid NOT IN (
    SELECT max(b.ctid)
    FROM public.assessments b
    WHERE b.technician_id IS NOT NULL
    GROUP BY b.technician_id, b.date
  );

CREATE UNIQUE INDEX IF NOT EXISTS assessments_technician_id_date_uidx
  ON public.assessments (technician_id, date);

DROP TABLE IF EXISTS public._ne_import_staging;
CREATE TABLE public._ne_import_staging (
...
INSERT INTO public._ne_import_staging (action, tech_name, assessment_date, safety_avg, basic_avg, intermediate_avg, advanced_avg, survey_avg, comment, raw_scores) VALUES
...
-- Historical + dated snapshots (no ON CONFLICT — works with tech_id or technician_id)
SELECT app_upsert_assessment_for_tech(
  app_match_technician_id(i.tech_name),
  i.assessment_date,
  i.safety_avg,
  i.basic_avg,
  i.intermediate_avg,
...
FROM public._ne_import_staging i
WHERE i.action = 'upsert'
  AND i.assessment_date IS NOT NULL
  AND app_match_technician_id(i.tech_name) IS NOT NULL;
...
SELECT 'dom_jean_louis' AS check_name, t.name, t.region, a.date, a.safety_avg, a.basic_avg
FROM public.technicians t
JOIN public.assessments a ON a.technician_id = t.id
WHERE lower(trim(t.name)) = 'dom jean-louis'
ORDER BY a.date DESC;

SELECT i.tech_name, app_match_technician_id(i.tech_name) AS matched_tech_id
FROM (SELECT DISTINCT tech_name FROM public._ne_import_staging) i
ORDER BY i.tech_name;

SELECT 'june_2026_imported' AS check_name, count(*) AS row_count
FROM public.assessments a
JOIN public.technicians t ON t.id = a.technician_id
WHERE t.region = 'NE' AND t.deleted_at IS NULL AND a.date >= '2026-06-01';

DROP TABLE public._ne_import_staging;
